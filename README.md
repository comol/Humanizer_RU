...
Claude Code — скопировать папку:

```powershell
Copy-Item -Recurse skills\humanizer-ru ~\.claude\skills\humanizer-ru      # глобально
Copy-Item -Recurse skills\humanizer-ru .claude\skills\humanizer-ru        # в проект
```

Дальше в любом агенте: «очеловечь», «сделай читаемым», «убери канцелярит», «проверь на слоп». Скилл сам запустит линтер, если есть python, пройдёт по каталогу и вернёт текст + «что изменено» + «на решение автора» + «нужны данные».
